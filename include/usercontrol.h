...
void Up_down()
{
    while (true)
    {
        ...
        {
            Collect_Bottom.spin(reverse, Stop_Collect_Bottom * 0.8, velocityUnits::pct);
            Collect_Top.spin(forward, Stop_Collect_Top * 0.8, velocityUnits::pct);
        }
        else if (Controller1.ButtonR2.pressing())
        {
            Collect_Bottom.spin(reverse, -1 * Stop_Collect_Bottom * 0.8, velocityUnits::pct);
            Collect_Top.spin(forward, -1 * Stop_Collect_Top * 0.8, velocityUnits::pct);
        }
        else if (switch_ == false)
        {
            Collect_Bottom.stop();
